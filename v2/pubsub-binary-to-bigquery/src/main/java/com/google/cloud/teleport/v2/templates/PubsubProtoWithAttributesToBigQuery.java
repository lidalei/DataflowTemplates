...
import com.ridedott.iot.schema.telemetry.Telemetry;
 ...
 */
public final class PubsubProtoWithAttributesToBigQuery {
  ...
  private static PipelineResult run(PubSubProtoToBigQueryOptions options) {
    ...
    PCollection<String> maybeForUdf =
        pipeline
              .apply("Read From Pubsub", PubsubIO.readMessagesWithAttributes())
              .apply("Dynamic Message to TableRow", new ConvertPubSubProtoMessageToJson(options));
   ...
   *
   * <p>The BigQuery schema should be defined by the JSON schema path in {@code options}.
   */
  @VisibleForTesting
  ...
  private static class ConvertPubSubProtoMessageToJson
      extends PTransform<PCollection<PubsubMessage>, PCollection<String>> {
    private final boolean preserveProtoName;

    private ConvertPubSubProtoMessageToJson(PubSubProtoToBigQueryOptions options) {
              ...
              .via(
                  message -> {
                    try {
                      ...
                      Telemetry telemetryMessage = Telemetry.parseFrom(message.getPayload());
                      String data = preserveProtoName
                              ? printer.preservingProtoFieldNames().print(telemetryMessage)
                              : printer.print(telemetryMessage);
                      ...
                      if (telemetryAttributes != null) {
                        for (Map.Entry<String, String> entry : telemetryAttributes.entrySet()) {
                          ...
                          if (key.equals("vehicleId")) {
                            ...
                            completeData.put("received_at", Instant.ofEpochMilli(Long.parseLong(value)).toString());
                          }
                        }

                        completeData.put("attributes", new ObjectMapper().writeValueAsString(telemetryAttributes));
                      }

                      return new ObjectMapper().writeValueAsString(completeData);
                    } catch (InvalidProtocolBufferException | JsonProcessingException | DateTimeException e) {
